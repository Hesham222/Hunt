...
class FilterAction
{
    ...
    {
        return AccountReport::when($request->input('view') == 'trash', function ($query) use ($request) {
        ...
        }])->with(['ReportedUser' => function ($query) use ($request) {
                $query->select(['id','first_name','last_name','email','phone']);
        }])
        ->select(['id','broker_id','developer_id','individual_id','reported_id','account_report_reason_id','status','other_reason','comments','deleted_by','deleted_at','created_at'])
        ...
        ->when($request->input('column') && $request->input('value'), function ($query) use ($request){
            ...
            return $query->where('account_report_reason_id',  $request->input('reason') );
