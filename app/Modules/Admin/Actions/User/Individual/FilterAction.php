...
class FilterAction
{
    ...
    {
        return Individual::with('posts')->when($request->input('view') == 'trash', function ($query) use ($request) {
        ...
        ->select(['id','first_name','last_name','email','status','date_of_birth','image', 'phone','deleted_by','deleted_at','created_by', 'created_at'])
        ...
        ->when($request->input('column') && $request->input('value'), function ($query) use ($request){
            ...
            return $query->where('id', $request->input('individual'));

        })->when($request->input('individualName'), function ($query) use ($request) {
                return $query->where('first_name', $request->input('individualName'));
        });
