...
class CreateAccountReportsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('account_reports', function (Blueprint $table) {
            ...
            $table->foreign('reported_id')->references('id')->on('individuals');
            $table->enum('status', ['Pending', 'Dismissed'])->default('Pending');
            $table->bigInteger('account_report_reason_id')->unsigned()->nullable();;
            $table->foreign('account_report_reason_id')->references('id')->on('account_report_reasons');
