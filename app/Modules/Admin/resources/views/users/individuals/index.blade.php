...
@if(request()->input('view')=='trash')
  <x-slot name="pageTitle">Individuals | Trash</x-slot name="pageTitle">
  @section('individuals-trash-active', 'm-menu__item--active')
@elseif(request()->input('view')=='blocked')
  <x-slot name="pageTitle">Individuals | Blocked</x-slot name="pageTitle">
  @section('individuals-blocked-active', 'm-menu__item--active')
@elseif(request()->input('view')=='verified')
  <x-slot name="pageTitle">Individuals | Verified</x-slot name="pageTitle">
  @section('individuals-verified-active', 'm-menu__item--active')
@elseif(request()->input('view')=='notVerified')
  <x-slot name="pageTitle">Individuals | Not Verified</x-slot name="pageTitle">
  @section('individuals-notVerified-active', 'm-menu__item--active')
@else
  <x-slot name="pageTitle">Individuals | View All</x-slot name="pageTitle">
  @section('individuals-view-active', 'm-menu__item--active')
@endif
        ...
        <div class="m-portlet m-portlet--mobile">
            ...
            <div class="m-portlet__head-tools">
              <ul class="m-portlet__nav">
                <li class="m-portlet__nav-item">
                  <a href="{{route('admins.individual.create')}}"
          ...
          <div class="m-portlet__body">
             <section class="content">
                @include('Admin::users.individuals.components.filterForm')
...
<x-slot name="scripts">
  <script>
    $(function () {
        render("{!! route('admins.individual.data',['individual' => request()->input('individual',0),'view' => request()->input('view',0)]) !!}");
        /**
        ...
        $('#searchButton').on('click', function (event) {
            event.stopPropagation();
            render("{!! route('admins.individual.data',['individual' => request()->input('individual',0),'view' => request()->input('view',0)]) !!}&column=" + $('#searchColumn').val() + '&value=' + $('#searchField').val()+ '&start_date=' + $('#startDateCol').val()+ '&end_date=' + $('#endDateCol').val());
        });
    });
  </script>
  <!-- external JS -->
