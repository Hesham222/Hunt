...
<tr id="tableRecord-{{$record->id}}">
    ...
    <td>
        <a
            href="{{route('admins.listing.index').'?developer='.$record->id}}"
            title="Listings"
            style="margin:5px"
            class="btn btn-sm btn-info">
            <span class="m-menu__link-badge">
                <span class="m-badge m-badge--danger" >
                    {{$record->listings()->count()}}
                </span>
            </span>
    ...
    @if(request()->query('view')=='trash')
        ...
        onclick="injectModalData('{{$record->id}}', '{{route('admins.developer.restore')}}', 'confirm-password-form', 'POST')"
        >
        ...
        <a
            ...
            data-toggle="modal"
            data-target="#confirm-password-modal"
            onclick="injectModalData('{{$record->id}}', '{{route('admins.developer.destroy', $record->id)}}', 'confirm-password-form', 'DELETE')"
        >
    ...
    @else
        ...
        <a 
            ...
            data-toggle="modal" 
            data-target="#confirm-password-modal"
            onclick="injectModalData('{{$record->id}}', '{{route('admins.developer.block')}}', 'confirm-password-form', 'POST')" >
        ...
        <a 
            ...
            onclick="injectModalData('{{$record->id}}', '{{route('admins.developer.reset.password')}}', 'reset-users-password-form', 'POST')"
            >
            <i class="fa fa-key" style="color: #fff"></i>
        </a>
        {{-- Edit--}}
        <a
            href="{{route('admins.developer.edit',$record->id)}}"
            title="Edit"
            style="margin:5px"
            class="btn btn-sm btn-primary">
        ...
        data-toggle="modal"
        data-target="#confirm-password-modal"
        onclick="injectModalData('{{$record->id}}', '{{route('admins.developer.trash')}}', 'confirm-password-form', 'POST')" >
        <i class="fa fa-trash" style="color: #fff"></i>
        </a>
    @endif
    </td>
