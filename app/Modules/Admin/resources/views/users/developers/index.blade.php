...
@if(request()->input('view')=='trash')
  <x-slot name="pageTitle">Developers | Trash</x-slot name="pageTitle">
  @section('developers-trash-active', 'm-menu__item--active')
@elseif(request()->input('view')=='blocked')
  <x-slot name="pageTitle">Developers | Blocked</x-slot name="pageTitle">
  @section('developers-blocked-active', 'm-menu__item--active')
@elseif(request()->input('view')=='verified')
  <x-slot name="pageTitle">Developers | Verified</x-slot name="pageTitle">
  @section('developers-verified-active', 'm-menu__item--active')
@elseif(request()->input('view')=='notVerified')
  <x-slot name="pageTitle">Developers | Not Verified</x-slot name="pageTitle">
  @section('developers-notVerified-active', 'm-menu__item--active')
@else
  <x-slot name="pageTitle">Developers | View All</x-slot name="pageTitle">
  @section('developers-view-active', 'm-menu__item--active')
@endif
@section('developers-active', 'm-menu__item--active m-menu__item--open')
        ...
        <div class="m-portlet m-portlet--mobile">
            ...
            <div class="m-portlet__head-tools">
              <ul class="m-portlet__nav">
                <li class="m-portlet__nav-item">
                  <a href="{{route('admins.developer.create')}}"
          ...
          <div class="m-portlet__body">
             <section class="content">
                @include('Admin::users.developers.components.filterForm')
...
<x-slot name="scripts">
  <script>
    $(function () {
        render("{!! route('admins.developer.data',['view' => request()->input('view',0)]) !!}");
        /**
        ...
        $('#searchButton').on('click', function (event) {
            event.stopPropagation();
            render("{!! route('admins.developer.data',['view' => request()->input('view',0)]) !!}&column=" + $('#searchColumn').val() + '&value=' + $('#searchField').val()+ '&start_date=' + $('#startDateCol').val()+ '&end_date=' + $('#endDateCol').val());
        });
    });
  </script>
  <!-- external JS -->
