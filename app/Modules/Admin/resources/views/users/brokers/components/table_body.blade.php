...
<tr id="tableRecord-{{$record->id}}">
    ...
    <td>
        <a
            href="{{route('admins.listing.index').'?broker='.$record->id}}"
            title="Listings"
            style="margin:5px"
    ...
    @if(request()->query('view')=='trash')
        ...
        onclick="injectModalData('{{$record->id}}', '{{route('admins.broker.restore')}}', 'confirm-password-form', 'POST')"
        >
        ...
        <a
            ...
            data-toggle="modal"
            data-target="#confirm-password-modal"
            onclick="injectModalData('{{$record->id}}', '{{route('admins.broker.destroy', $record->id)}}', 'confirm-password-form', 'DELETE')"
        >
    ...
    @else
        ...
        <a 
            ...
            data-toggle="modal" 
            data-target="#confirm-password-modal"
            onclick="injectModalData('{{$record->id}}', '{{route('admins.broker.block')}}', 'confirm-password-form', 'POST')" >
        ...
        <a 
            ...
            onclick="injectModalData('{{$record->id}}', '{{route('admins.broker.reset.password')}}', 'reset-users-password-form', 'POST')"
            >
            <i class="fa fa-key" style="color: #fff"></i>
        </a>
        {{-- Edit--}}
        <a
            href="{{route('admins.broker.edit',$record->id)}}"
            title="Edit"
            style="margin:5px"
            class="btn btn-sm btn-primary">
        ...
        data-toggle="modal"
        data-target="#confirm-password-modal"
        onclick="injectModalData('{{$record->id}}', '{{route('admins.broker.trash')}}', 'confirm-password-form', 'POST')" >
        <i class="fa fa-trash" style="color: #fff"></i>
        </a>
    @endif
    </td>
