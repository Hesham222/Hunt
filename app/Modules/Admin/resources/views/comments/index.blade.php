...
@if(request()->input('view')=='trash')
  <x-slot name="pageTitle">Comment | Trash</x-slot name="pageTitle">
  @section('comments-trash-active', 'm-menu__item--active')
@else
  <x-slot name="pageTitle">Comment | View</x-slot name="pageTitle">
  @section('comments-view-active', 'm-menu__item--active')
@endif
...
{{--                  <a href="{{route('admins.comment.create')}}"--}}
...
{{--                </li>--}}
          ...
          <div class="m-portlet__body">
             <section class="content">
                @include('Admin::comments.components.filterForm')
...
<x-slot name="scripts">
  <script>
    $(function () {
        render("{!! route('admins.comment.data',['view' => request()->input('view',0)]) !!}");
        /**
        ...
        $('#searchButton').on('click', function (event) {
            event.stopPropagation();
            render("{!! route('admins.comment.data',['view' => request()->input('view',0)]) !!}&column=" + $('#searchColumn').val() + '&value=' + $('#searchField').val()+ '&start_date=' + $('#startDateCol').val()+ '&end_date=' + $('#endDateCol').val());
        });
    });
  </script>
  <!-- external JS -->
