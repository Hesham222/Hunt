...
<tr id="tableRecord-{{$record->id}}">
    ...
    <td>{{$record -> post_id ?  $record->post_id : ""}}</td>
    <td>
    ...
    <td>
        @if($record->status == "Pending")
            <a
                ...
                onclick="injectModalData('{{$record->id}}', '{{route('admins.postReport.dismiss')}}', 'confirm-password-form', 'POST')"
            >
    ...
    @if(request()->query('view')=='trash')
        ...
        style="margin:5px"
        data-target="#confirm-password-modal"
        onclick="injectModalData('{{$record->id}}', '{{route('admins.postReport.restore')}}', 'confirm-password-form', 'POST')"
        >
        ...
        <a
            ...
            style="margin:5px"
            data-target="#confirm-password-modal"
            onclick="injectModalData('{{$record->id}}', '{{route('admins.postReport.destroy', $record->id)}}', 'confirm-password-form', 'DELETE')"
        >
            <i class="fa fa-trash" style="color: #fff"></i>
        </a>
    @else
        <a
            href="{{route('admins.post.index').'?view=view&post='.$record -> post_id}}"
            title="Show Reported Post"
            style="margin:5px"
            class="btn btn-sm btn-primary">
        <i class="far fa-file-alt" style="color: #fff"></i>
            ...
            data-toggle="modal"
            data-target="#confirm-password-modal"
            onclick="injectModalData('{{$record->id}}', '{{route('admins.postReport.trash')}}', 'confirm-password-form', 'POST')" >
            <i class="fa fa-trash" style="color: #fff"></i>
            </a>
    @endif
    </td>
